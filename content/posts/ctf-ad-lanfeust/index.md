...
`darshan.lab` had been visible since the first sweep and ignored for six hours. This is the leg where Retro did most of the driving, and I am reconstructing it from the tickets he handed over plus his notes, so I want to get the direction right, because my first published version got it backwards and two readers rightly called it out.

The trust is one way and outbound from troy: **`troy.lab` trusts `darshan.lab`**. In Microsoft's terms darshan is the *trusted* domain and troy is the *trusting* one, which normally means darshan principals can be presented to troy and not the other way around. We went the other way, into darshan, and that is the whole point.

The reason it works is the trust account. When you create a one way trust, a matching account named after the *trusting* domain, here `TROY$`, is created inside the *trusted* domain, `darshan.lab`, and both sides share its secret. Control the trusting side, which we did, and you can read that secret out of troy's own Trusted Domain Object and then simply log in to darshan **as `TROY$`**. Almond's ["are one-way trusts really one-way"](https://offsec.almond.consulting/trust-no-one_are-one-way-trusts-really-one-way.html) is the writeup on exactly this, and impacket's [PR #2288](https://github.com/fortra/impacket/pull/2288) is what makes `secretsdump` derive the trust-account keys (with the NetBIOS-flat salt, not just the inter-realm FQDN one) so `getTGT` can use them directly.

So this is not a forged inter-realm referral presented as a foreign principal, which is what I wrote before. It is a plain AS-REQ: `TROY$` authenticating into darshan with its own key and getting a normal TGT. The ticket Retro produced proves it, the flags on it read `initial, pre_authent` and the service is `krbtgt/DARSHAN.LAB`, i.e. a genuine login, not a crafted ticket:

```bash
# derive the trust-account keys from the TDO secret (impacket >= PR #2288)
secretsdump.py -just-dc-user 'troy.lab\darshan$' 'TROY.LAB/...@eckmul.troy.lab'
# then log in to the TRUSTED domain as the trust account
getTGT.py -aesKey <TROY$ trust key> darshan.lab/'TROY$'
```

Retro supplied the resulting `darshan-trust.ccache`. From there `darshan.lab` runs on real time, so this is one of the places the `faketime` prefix has to come off:
...
# Service: krbtgt/DARSHAN.LAB@DARSHAN.LAB   (a real TGT, flags: initial, pre_authent)
...
![Recap: authenticating into the trusted domain as the trust account, then reading a share on the other side.](chain-09.png)
*(kill chain generated by [Exegol Studio](https://exegol.com/studio).)*
...
That is `DARSHAN\Administrator`, except an LDAP shell cannot pull a file off `C$` by itself. The last step is the standard bridge: from the shell, drop a controlled account into `Domain Admins` for a moment, read `C:\flags\flag12.txt` over SMB as that now-privileged account, then take it straight back out of the group. Retro walked me through this and handed over the artefacts so I could follow along, which is the only reason this cross-domain half is in the post at all: teamwork makes the dream work.
...
I am going to get Retro's full notes as soon as I can and come back to tighten this whole darshan section, exact accounts and commands included, rather than leave any of it at the level of "roughly this".

12/12 at 19:50.

![Recap: ESC7 on DARSHAN-CA, an administrator-SAN certificate, DARSHAN\\Administrator over ldap-shell.](chain-12.png)
*(kill chain generated by [Exegol Studio](https://exegol.com/studio).)*
...
- [Are one-way trusts really one-way?](https://offsec.almond.consulting/trust-no-one_are-one-way-trusts-really-one-way.html), Almond Offensive Security
- [impacket PR #2288](https://github.com/fortra/impacket/pull/2288), trust-account key derivation for `getTGT`
