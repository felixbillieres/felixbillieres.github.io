...
images: ["image.png", "forceChangePassword.png", "addAllowedToAct.png"]
---
...
3. [Phase 2: Password Discovery in PDF](#phase-2-password-discovery-in-pdf)
4. [Phase 3: User Enumeration and Password Spray](#phase-3-user-enumeration-and-password-spray)
5. [Phase 4: Initial Access with ibryant](#phase-4-initial-access-with-ibryant)
6. [Phase 5: VeraCrypt Container Discovery](#phase-5-veracrypt-container-discovery)
7. [Phase 6: Cracking the VeraCrypt Container](#phase-6-cracking-the-veracrypt-container)
8. [Phase 7: VyOS Credentials Extraction](#phase-7-vyos-credentials-extraction)
9. [Phase 8: Access with svc_sspr and user.txt](#phase-8-access-with-svc_sspr-and-usertxt)
10. [Phase 9: Privilege Escalation via RBCD](#phase-9-privilege-escalation-via-rbcd)
11. [Phase 10: Complete Domain Compromise](#phase-10-complete-domain-compromise)
...
The `Public` share is accessible in read mode with the Guest account. Let's use NetExec's `spider_plus` module to automatically download files:
...
## Phase 2: Password Discovery in PDF
...
## Phase 3: User Enumeration and Password Spray
...
## Phase 4: Initial Access with ibryant
...
## Phase 5: VeraCrypt Container Discovery
...
## Phase 6: Cracking the VeraCrypt Container
...
## Phase 7: VyOS Credentials Extraction
...
## Phase 8: Access with svc_sspr and user.txt
...
## Phase 9: Privilege Escalation via RBCD
...
**Resource-Based Constrained Delegation (RBCD)** is a Kerberos delegation mechanism introduced in Windows Server 2012. Unlike traditional constrained delegation, RBCD is configured on the **target resource** (the service account) rather than on the **delegating account**.

**Key Concepts:**

1. **msDS-AllowedToActOnBehalfOfOtherIdentity**: This attribute on the target service account specifies which accounts can delegate to it.

2. **S4U2Self + S4U2Proxy**: The delegation process uses two Kerberos extensions:
   - **S4U2Self (Service for User to Self)**: Allows a service to obtain a ticket for itself on behalf of a user.
   - **S4U2Proxy (Service for User to Proxy)**: Allows a service to obtain a ticket for another service on behalf of a user.

3. **Traditional RBCD Attack Flow:**
   - Create a machine account (requires `MachineAccountQuota > 0`)
   - Configure RBCD on the target to allow delegation from the machine account
   - Use S4U2Self + S4U2Proxy to impersonate any user

### The SPN-less User Technique

When `MachineAccountQuota` is set to 0, we can't create new machine accounts. However, James Forshaw discovered that **regular user accounts without SPNs** can be used for RBCD exploitation.

**Why This Works:**

- The Kerberos protocol doesn't strictly require SPNs for delegation
- User accounts can be added to `msDS-AllowedToActOnBehalfOfOtherIdentity`
- The S4U2Self+U2U (User-to-User) extension allows using a user account's TGT session key

**Important Caveat:** This technique modifies the user account's password hash, potentially rendering the account unusable for normal operations. This is why it should be avoided on production accounts.

### Configuring RBCD Delegation

Let's configure RBCD to allow `wsilva` to impersonate other users on `DC$`:
...
### Obtaining a TGT for wsilva

Let's obtain a Kerberos ticket (TGT) for `wsilva`. First, we need to change the password back to a known value since we'll need to manipulate it:
...
1. **`-u2u`**: Enables User-to-User authentication, allowing the use of the TGT's session key
2. **`-impersonate Administrator`**: Requests a ticket for the Administrator account
3. **`-spn cifs/DC.phantom.vl`**: Specifies the target service (CIFS on the domain controller)
4. **`-k`**: Uses keytab mode (uses the TGT's session key instead of password)
5. **`-no-pass`**: Don't prompt for password (we're using the keytab)

**Success!** We now have a Kerberos ticket allowing us to impersonate `Administrator` on the domain controller.

---

## Phase 10: Complete Domain Compromise
...
**Technical Deep Dive - RBCD Attack Chain:**
